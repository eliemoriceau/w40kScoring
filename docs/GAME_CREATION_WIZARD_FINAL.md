    ...
    <div class="players-config">
      ...
      <div class="player-card player-1">
        ...
        <div class="player-fields">
          <div class="field">
            <label>Pseudo</label>
            <input type="text" :value="currentUser.pseudo" disabled class="input-disabled" />
          </div>
          <div class="field">
            <label>Armée</label>
            <input
      ...
      <div class="player-card player-2">
        ...
        <div class="player-fields">
          <div class="field">
            <label>Pseudo</label>
            <input type="text" :value="getOpponentPseudo()" disabled class="input-disabled" />
          </div>
          <div class="field">
            <label>Armée</label>
            <input
...
const isStep3Valid = computed(() => {
  return (
    data.value.players.length === 2 && data.value.players.every((p) => p.pseudo?.trim().length > 0)
  )
})
</script>
```
...
<template>
  <div class="step-container step-4">
    ...
    <StepNavigation :can-continue="true" @previous="$emit('previous')" @next="$emit('next')" />
  </div>
</template>
...
<template>
  <div class="round-card">
    ...
    <div v-if="playerScore || opponentScore" class="round-winner">
      ...
      <span v-else class="tie"> 🤝 Égalité </span>
    </div>
  </div>
</template>
...
export default class PartiesController {
  ...
  private async sendGameInvitation(data: { email: string; inviterName: string; gameDetails: any }) {
