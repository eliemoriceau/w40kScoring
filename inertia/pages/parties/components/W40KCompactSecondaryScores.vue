  ...
  <div class="space-y-4">
    ...
    <div class="bg-slate-800 border border-red-800/50 rounded-lg p-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="space-y-2">
          <h5 class="text-sm font-medium text-red-300 border-b border-slate-600 pb-1">
            ...
            <span class="text-xs text-slate-400"
              >(Total: {{ getPlayerSecondaryTotal(players[0]) }})</span
            >
          </h5>
          ...
          <div class="border border-slate-600 rounded-lg overflow-hidden">
            ...
            <div class="text-xs">
              ...
              <template v-for="(row, index) in getPlayerRows(players[0])" :key="row.id">
                <div
                  class="grid grid-cols-12 border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors items-center"
                >
                  ...
                  <div class="col-span-9 p-2 border-r border-slate-600">
                    ...
                    >
                      ...
                      <optgroup label="🔥 Tactiques">
                        <option
                          v-for="obj in tacticalObjectives"
                          :key="obj.value"
                          :value="obj.value"
                        >
                      ...
                      <optgroup label="🧠 Stratégiques">
                        <option
                          v-for="obj in strategicObjectives"
                          :key="obj.value"
                          :value="obj.value"
                        >
                          {{ obj.label }} ({{ obj.maxPoints }}pts)
        ...
        <div v-if="players[1]" class="space-y-2">
          <h5 class="text-sm font-medium text-red-300 border-b border-slate-600 pb-1">
            {{ players[1].pseudo }}
            <span class="text-xs text-slate-400"
              >(Total: {{ getPlayerSecondaryTotal(players[1]) }})</span
            >
          </h5>
      ...
      <div v-if="allSecondaryScores.length === 0" class="text-center py-4 text-slate-400 text-sm">
        <svg
          class="w-8 h-8 mx-auto mb-2 text-slate-500"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
...
const tacticalObjectives: SecondaryObjectiveOption[] = [
  {
    value: 'engage-all-fronts',
    label: 'Engagement Sur Tous Les Fronts',
    category: 'tactical',
    maxPoints: 15,
  },
  { value: 'linebreaker', label: 'Briseur de Ligne', category: 'tactical', maxPoints: 15 },
  {
    ...
    label: 'Derrière les Lignes Ennemies',
    category: 'tactical',
    maxPoints: 15,
  },
  {
    value: 'teleport-homer',
    label: 'Balise de Téléportation',
    category: 'tactical',
    maxPoints: 15,
  },
]
...
const strategicObjectives: SecondaryObjectiveOption[] = [
  ...
  {
    value: 'investigate-sites',
    label: 'Enquêter Sur Les Sites',
    category: 'strategic',
    maxPoints: 15,
  },
...
const wardenObjectives: SecondaryObjectiveOption[] = [
  {
    value: 'defend-stronghold',
    label: 'Défendre la Forteresse',
    category: 'warden',
    maxPoints: 15,
  },
