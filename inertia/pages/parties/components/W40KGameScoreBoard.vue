<template>
  <div class="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="relative bg-gradient-to-br from-w40k-bg-secondary to-w40k-bg-elevated border-2 border-w40k-red-500/50 rounded-xl p-6 shadow-w40k-xl overflow-hidden">
      ...
      <div class="relative grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        ...
        <div class="text-center md:text-left space-y-4">
          <div class="space-y-2">
            <h3 class="text-lg font-bold text-w40k-red-300 flex items-center justify-center md:justify-start gap-2">
            ...
            <div class="relative">
              <div class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-w40k-red-400 to-w40k-gold-400 drop-shadow-lg">
                {{ getPlayerTotalScore(players[0]) }}
              </div>
              <div v-if="getPlayerTotalScore(players[0]) > 30" class="absolute inset-0 bg-gradient-to-r from-w40k-gold-400/20 to-w40k-red-400/20 blur-xl animate-pulse" />
            </div>
            ...
            <div class="flex flex-col gap-1 text-sm">
              <div class="flex items-center justify-center md:justify-start gap-2">
                ...
                <span class="text-blue-300 font-medium">Primaires: {{ getPrimaryScore(players[0]) }}</span>
              </div>
              <div class="flex items-center justify-center md:justify-start gap-2">
                <div class="w-2 h-2 bg-green-400 rounded-full" />
                <span class="text-green-300 font-medium">Secondaires: {{ getSecondaryScore(players[0]) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="text-center space-y-4">
          <div class="relative">
            <div class="text-3xl font-black text-w40k-text-subtle bg-w40k-bg-primary px-6 py-3 rounded-full border-2 border-w40k-text-subtle/30 shadow-inner">
              VS
            </div>
            <div class="absolute -inset-1 bg-gradient-to-r from-w40k-red-500/20 via-w40k-gold-500/20 to-w40k-red-500/20 rounded-full blur animate-pulse" />
          </div>
          ...
          <div v-if="game.winner" class="animate-in zoom-in-50 duration-700">
            <div class="relative">
              ...
              <div v-if="isMainPlayerWinner" class="absolute -inset-2 bg-gradient-to-r from-green-500/30 to-green-400/30 rounded-lg blur-sm animate-pulse" />
            </div>
          </div>
        </div>
        ...
        <div v-if="players[1]" class="text-center md:text-right space-y-4">
          <div class="space-y-2">
            <h3 class="text-lg font-bold text-w40k-red-300 flex items-center justify-center md:justify-end gap-2">
              <span class="w-3 h-3 bg-w40k-red-500 rounded-full animate-pulse" />
              {{ players[1].pseudo }}
            ...
            <div class="relative">
              <div class="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-l from-w40k-red-400 to-w40k-gold-400 drop-shadow-lg">
                {{ getPlayerTotalScore(players[1]) }}
              </div>
              <div v-if="getPlayerTotalScore(players[1]) > 30" class="absolute inset-0 bg-gradient-to-l from-w40k-gold-400/20 to-w40k-red-400/20 blur-xl animate-pulse" />
            </div>
            
            <div class="flex flex-col gap-1 text-sm">
              <div class="flex items-center justify-center md:justify-end gap-2">
                <div class="w-2 h-2 bg-blue-400 rounded-full" />
                <span class="text-blue-300 font-medium">Primaires: {{ getPrimaryScore(players[1]) }}</span>
              </div>
              <div class="flex items-center justify-center md:justify-end gap-2">
                <div class="w-2 h-2 bg-green-400 rounded-full" />
                <span class="text-green-300 font-medium">Secondaires: {{ getSecondaryScore(players[1]) }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="bg-gradient-to-br from-w40k-bg-secondary to-w40k-bg-elevated border-2 border-w40k-red-500/50 rounded-xl p-6 shadow-w40k-lg">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <h3 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-w40k-red-300 to-w40k-gold-400">
          ⚔️ Scores par Round
        </h3>
      ...
      <div v-if="localRounds.length === 0" class="py-16 text-center">
        <div class="space-y-4">
          <div class="w-16 h-16 mx-auto bg-gradient-to-br from-w40k-red-500/20 to-w40k-gold-500/20 rounded-full flex items-center justify-center">
            <svg class="w-8 h-8 text-w40k-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
          </div>
          <div class="space-y-2">
            <p class="text-w40k-text-muted text-lg">Aucun round créé pour cette partie</p>
            <p class="text-sm text-w40k-text-subtle">Les rounds seront automatiquement créés lors du démarrage</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showProgressIndicators" class="bg-gradient-to-br from-w40k-bg-secondary to-w40k-bg-elevated border-2 border-w40k-red-500/30 rounded-xl p-6">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-3">
          ...
          <div class="relative w-full bg-w40k-bg-primary rounded-full h-3 overflow-hidden">
            ...
            >
              <div class="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent animate-pulse" />
            </div>
          </div>
          
          <span class="text-xs text-w40k-text-muted">{{ getCompletedRounds() }}/{{ localRounds.length }}</span>
        </div>
        ...
        <div class="space-y-3">
          ...
          <span class="text-xs text-w40k-text-muted">{{ getEnteredScores() }}/{{ getTotalPossibleScores() }}</span>
        </div>
      </div>
    </div>
    ...
    >
      <div v-if="showNotification" :class="notificationClasses" class="fixed bottom-6 right-6 z-50 px-6 py-4 rounded-lg shadow-2xl border-2 max-w-sm">
...
const gameStatusText = computed(() => {
  switch (props.game.status) {
    ...
    case 'IN_PROGRESS': return '⚔️ En cours'
...
const gameStatusClasses = computed(() => {
  const baseClasses = 'inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold border-2 transition-all duration-200'
...
const isMainPlayerWinner = computed(() => {
  ...
  return (props.game.winner === 'PLAYER' && props.players[0]?.isMainPlayer) ||
